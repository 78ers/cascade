...
- **Деплой Caddyfile** (после правок `deploy.py`): **обязательно сохранять текущий bind-порт** — на мосту Caddy сидит на :8443 за nginx (§16), и генерация с дефолтом 443 уронит панель (порт занят nginx). Определять порт из текущего файла, а не задавать руками: `python3 -c 'from cascade.panel.deploy import caddyfile,PANEL_CADDY_PATH;from cascade.config import load_config,CONFIG_PATH;c=load_config(CONFIG_PATH);cur=open(PANEL_CADDY_PATH).read();b=8443 if (c.domain+":8443") in cur else 443;open(PANEL_CADDY_PATH,"w").write(caddyfile(c.domain,c.panel.port,caddy_bind_port=b));print("bind:",b)'` затем `caddy validate --config /etc/caddy/Caddyfile --adapter caddyfile` и только при успехе `systemctl reload caddy`.
...
- **Клиентский профиль** (`build_client_xray_config`): split-туннель РФ→direct, прочее→каскад. host = IP моста (РФ). split-DNS: РФ → Яндекс 77.88.8.8, прочее → Google DoH `https://8.8.8.8/dns-query` (IP, не домен — иначе bootstrap через системный DNS провайдера), безфильтровый `77.88.8.8` последним как fail-open резерв. Только Xray-core клиенты (Happ/OneXray/v2rayNG/Nekoray/NekoBox); Karing (sing-box) отброшен.
...
- Клиентский профиль на устройстве: РФ-сайт → РФ IP (2ip.ru), иностранное → выход; direct-профиль (минуя мост). Bootstrap DoH закрыт: DoH задан IP.
...
## Сессия 2026-06-26 (РФ-TLD direct + полнота WeChat + RAM Happ)
- Тесты **180/180**. Полная картина — PROJECT.md §19. Коммит `2f40fbf`.
- **РФ-сайты с отозванными сертификатами (Минцифры) не открывались под VPN.** По дизайну все РФ→direct, но урезанный `geosite.dat` в Happ часть РФ-доменов не ловит → утекали в catch-all→proxy→иностранный выход→TLS рвётся ДО показа сертификата (симптом «не открывается, без сообщения»; без VPN — обходимое предупреждение). Фикс: правило `domain:ru`/`domain:su`/`domain:xn--p1ai` (.рф) → direct + отдельная РФ-DNS запись (Яндекс 77.88.8.8, БЕЗ expectIPs — .ru на иностранном CDN тоже резолвится). Немануально, ловит любой .ru/.рф/.su, ~0 RAM. `geosite:category-ru` оставлен (РФ-сервисы на .com/.net).
...
## Сессия 2026-09-07 (клиентский DNS: DoH по IP + fail-open резерв)
- Тесты **194/194**. Полная картина — PROJECT.md §22. Правка только в `build_client_xray_config`, секция `dns.servers`; сервер/мост/роутинг/ключи не тронуты.
- **DoH задавать IP, не доменом.** Было `https://dns.google/dns-query` — домен не подпадал ни под одно правило `dns.servers` и секции `hosts` нет, значит Xray резолвил его системным DNS оператора (ТСПУ подменяет) → DoH не поднимался вовсе. Стало `https://8.8.8.8/dns-query`. `hosts` как подпорка отклонён: по докам Xray для DoH-домена адрес в local-режиме резолвится системой мимо `hosts`.
- **Безфильтровый `77.88.8.8` — ПОСЛЕДНИМ в `servers`.** Порядок критичен: Xray опрашивает безфильтровые серверы последовательно в порядке конфига, следующий берёт при провале предыдущего. Стоит последним → при живом туннеле не используется; поставить раньше DoH = зарубежные домены потекут российскому резолверу. Не переставлять.
- **Зачем резерв:** DoH ходит внутрь туннеля, а при `IPIfNonMatch` домен без совпадения по имени резолвится ДО проверки `geoip:ru` → РФ-сервисы на `.com` (нет в `.ru` и в урезанном `geosite.dat` Happ) умирали вместе с мостом. Теперь резолвятся Яндексом direct.
- **Принятый риск:** при мёртвом DoH Яндекс может вернуть РФ-адрес для зарубежного домена (РКН-заглушка/РФ-CDN) → правило direct по `geoip:ru` уведёт сайт мимо каскада. Только при уже неработающем туннеле. Симптом: «зарубежные сайты открываются неправильно».
- **Доезжает подпиской** (`/sub/<token>` генерит профиль на каждый запрос, интервал 3 ч). Ручной JSON — нужен переимпорт. `vless://`-ссылок не касается.
- **Мост отстал от переписанной истории — `git pull` не лечит.** 2026-09-07 деплой на мост упал: `+ <old>...<new> main -> origin/main (forced update)` и `fatal: Need to specify how to reconcile divergent branches`. Причина: историю репозитория переписывали от корня (подчистка реальных IP и Fake-TLS секрета), у моста и origin не осталось НИ ОДНОГО общего коммита — `git log --oneline origin/main..HEAD` уходил до корневого коммита. Сливать нечего, поэтому ни `--rebase`, ни `--ff-only`, ни `--no-rebase` не помогут. Диагностика перед лечением (только чтение): `git status --porcelain` (пусто = нет правок и неотслеживаемых файлов на проде) + `git diff --stat HEAD origin/main` (должны отличаться только ожидаемые файлы). Если оба чисты — `git reset --hard origin/main`. Конфиг `/etc/cascade/config.json` вне гита, ключи и клиенты не затрагиваются; перезапускать надо только `cascade-panel`. **Не предлагать `reset --hard` до этих двух проверок** — на мосту живой сервис.
- **Пейджер гита в SSH-сессии залипает** (`git log` открывает less, дальнейший ввод уходит в него, выглядит как зависание). Один раз на сервере: `git config --global core.pager cat`.
- **`direct` в клиентском профиле НЕ означает «мимо Happ».** При включённом клиенте iOS заворачивает весь трафик в его tun; российский пакет всё равно идёт через ядро Xray внутри Happ и лишь на развилке уходит в outbound `freedom`. Значит **Happ — единая точка отказа для всего трафика, включая РФ**: если ядро встало или клиент пересоздаёт туннель, потеряв основной outbound, до правил роутинга дело не доходит и не работает ничего. Наблюдалось вживую: при недоступном мосте не грузились и `.ru`, выключение Happ чинило мгновенно. Не искать причину в правилах роутинга, пока не исключён сам клиент. Тест и детали — PROJECT.md §22, «Открытый хвост».

## Сессия 2026-09-11 (панель на TLS 1.2)
- **Панель и подписка отдаются по TLS 1.2, без 1.3** (`caddyfile()` → блок `tls { protocols tls1.2 tls1.2 }`). Причина: часть РФ-операторов не доводит ClientHello от TLS 1.3 до сервера — домен не открывается без VPN, но открывается через него. Владелец наблюдал это вживую; совпадает с нашим разбором `docs/research/2026-07-01-tls-blocked-from-mobile.md` и с базой знаний AdminVPS (статья 559).
- **Reality это НЕ касается и касаться не должно.** VPN подключается по IP моста на высокий relay-порт (8444/8454/8474) через DNAT — ни домена, ни 443, ни nginx, ни Caddy на этом пути нет (`client_profile_json`: `host = relay_ip`). Reality построен на TLS 1.3 + X25519, отключать там 1.3 нельзя ни при каких условиях.
- **Следствие, объясняющее «подписка не обновляется»:** при таком операторе VPN у человека работает, а подписка по домену не скачивается — молча, без жалоб. Это альтернатива выводу «Happ не соблюдает интервал»: проверять по `sub_update_count` до и после правки.
- **HTTP/3 не трогали.** Caddy v2 поднимает его по умолчанию, но у нас на :443 стоит nginx stream (TCP), а UDP наружу не открыт — QUIC до Caddy не доходит. Отключать пришлось бы глобальным блоком в корневом Caddyfile, который мы принципиально не переписываем.
