...
- **dns** (split-DNS против leak):
  ...
  - прочее → Google DoH `https://8.8.8.8/dns-query` внутри туннеля (по IP, не домену — см. §22)
  - последним, безфильтровым — `77.88.8.8` direct: резерв, когда каскад лёг (§22)
...
- **Клиентский профиль на устройстве:** импорт в Happ/v2rayNG, РФ-сайт → РФ IP (2ip.ru), иностранное → выход; direct-профиль (минуя мост). Bootstrap DoH закрыт в §22 (DoH задан IP).
...
## 18. Изменения 2026-06-22 (бэкап конфига + deep-probe + альтернативные транспорты PoC)

**Тесты: 180/180.** Коммиты: `f0c8550` (hysteria + xhttp-профиль), `15a2269` (бэкап + монитор).

**Контекст.** Разбор аудит-документа от другой нейросети (`docs/research/2026-06-19-bridge-ram-audit-and-recommendations.md`) + консультации Grok/Gemini/NLM + живой тест на запасном выходе.

**Альтернативные транспорты — PoC и живой A/B. Вывод: текущий VLESS+Reality+raw TCP оставляем.**
- Happ поддерживает VLESS/VMess/Trojan/SS/Socks/**Hysteria2**; TUIC — нет. Единственный кандидат-апгрейд (а не даунгрейд от Reality) — Hysteria2.
- **XHTTP-клиентский профиль достроен** (`build_client_xray_config(xhttp_path=...)`, `client_xhttp_profile_json/url`, `vless_xhttp_reality_url`). Серверный xhttp-inbound + DNAT моста были раньше — дыра была только в клиентской генерации. Аддитивно: дефолт = прежний tcp+vision (`test_client_config_default_unchanged_is_tcp_vision`).
- **Живой A/B на ОДНОМ выходе (raw TCP vs XHTTP, одно время):** разницы нет — разброс между прогонами больше разницы транспортов. **raw-TCP-Reality НЕ задушен; XHTTP прироста не даёт** (вопреки тренду из консультаций — живьём подтвердил NLM, а не Grok/Gemini). Мигрировать незачем; XHTTP-код = страховка если ТСПУ начнёт точечно душить raw-TCP-fingerprint. Сплит-роутинг транспорт-независим (XHTTP-JSON несёт тот же роутинг/DNS; голая vless-ссылка — нет).
- **Hysteria2 (`cascade/hysteria.py`):** деплой по SSH (Salamander-обфускация, self-signed + pinSHA256, UDP) + генерация hy2-ссылки. Живой тест: сервер здоров (loopback-клиент работает), UDP проходит **и intra-RU, и трансгранично** (ТСПУ UDP НЕ блокирует), НО QUIC не достраивается до LTE-CGNAT-телефона (Happ «подключён, трафика нет»; асимметрия 129 vs 23 пакета). Консультанты (Gemini+NLM): причина = MTU (Salamander+LTE) и/или l4-25 packet-limit ТСПУ; фиксы = MTU~1280 на клиенте + UDP-zapret (nfqws на UDP-FORWARD) + app-релей (gost/realm) вместо iptables-DNAT. **Припаркован:** 3 доработки, payoff неясен, И в Happ Hysteria2 = URI-only → теряет наш сплит-роутинг. Как запасной XHTTP строго лучше.

**Аудит-рекомендации внедрены:**
- **§6.7 Бэкап конфига (`cascade/backup.py`):** `backup_config` (ротация, хранит 7) + `install_backup_cron` (ежесуточный cron.d, без CLI). Панель Настройки→Сервер: «⬇️ Скачать бэкап» (off-server DR-копия, `/boss/config/download`) + «💾 Бэкап сейчас + авто» (`/boss/config/backup`). Конфиг = единственный источник ключей/UUID/секретов; off-server копия → новый сервер с теми же ключами, ссылки клиентов выживают.
- **§6.6 deep-probe (`monitor.tls_handshake_ok`):** реальный TLS-хендшейк до выхода (ловит «слушает, но ТСПУ душит», что TCP-up пропускает). **§6.4 conntrack (`monitor.conntrack_usage`).** Оба в панели Диагностика → секция «Глубокая проверка». `run_check`/авто-рестарт НЕ тронуты (без риска ложных рестартов).
- RAM-аудит (применён владельцем на мосту): `fwupd` masked + journald vacuum (444→642 Mi available), `multipathd.socket` замаскирован. Swap/conntrack проблемой не были.

**Pitfalls/заметки:**
- Happ 4.11 выпилил `allowInsecure` → hy2-ссылка требует `pinSHA256=<отпечаток>`, не `insecure=1`.
- **Удаление выхода в панели (`exits_remove`) снимает ТОЛЬКО relay основного порта** — НЕ xhttp-порт (остаётся орфан-правило). CLI-версия (`menu.py`) чистит и xhttp-порт, и mtproto. TODO: дочистить панельную версию по образцу CLI.
- Hysteria2 раздаётся в Happ только URI-строкой (`hy2://`), в JSON не влезает → его профиль без сплит-роутинга. XHTTP в JSON влезает (роутинг сохранён).

## 19. Изменения 2026-06-26 (РФ-TLD direct + полнота WeChat + RAM Happ)

**Тесты: 180/180.** Коммит: `2f40fbf`. Правки только в `build_client_xray_config` (клиентский профиль) — сервер выхода/Xray не трогали.

**Контекст.** Жалоба: некоторые РФ-сайты с отозванными/Минцифровскими сертификатами не открываются под VPN (без VPN — обходимое предупреждение «принять риск»). Параллельно: память туннеля Happ выросла 14-16→29МБ при неизменном конфиге. Консультации: Grok + Gemini (напрямую через библиотеку — MCP висел) + сверка с эталоном `blackmatrix7/ios_rule_script` WeChat.yaml.

**РФ-сайты с сертификатами — TLD-правило (немануальный фикс).**
- По дизайну все РФ→direct (`geosite:category-ru` по домену + `geoip:ru` по IP). Но урезанный `geosite.dat` внутри Happ часть РФ-доменов не содержит → они утекали в catch-all→proxy→иностранный выход, где гео-блок/недоступность рвут TLS ДО показа сертификата (отсюда «не открывается, без сообщения»). DNS для них шёл через Google DoH в туннеле → иностранный CDN-IP → `geoip:ru` тоже мимо.
- Фикс: правило `domain:ru`/`domain:su`/`domain:xn--p1ai` (.рф) → `direct` + отдельная РФ-DNS запись (`77.88.8.8` без `expectIPs`). Ловит ЛЮБОЙ .ru/.рф/.su без ручного перечисления доменов, ~0 RAM. `geosite:category-ru` оставлен (РФ-сервисы на .com/.net).
- Trade-off: весь .ru/.рф/.su идёт direct с реального РФ-IP — это и есть заявленный дизайн «все РФ напрямую». Полное снятие предупреждения (а не только открытие) = установка корневого Russian Trusted Root CA на устройство (вне конфига VPN).

**WeChat — полнота (сверка blackmatrix7 + Grok + Gemini).**
- `WECHAT_DOMAINS` дополнен: `weixinbridge.com`, `weixinsxy.com`, `wechatlegal.net`, `iot-tencent.com`, `wechatpay.cn`, `tencentmap.com` + точечные `full:apd-pcdnwxlogin.teg.tencent-cloud.net`, `full:slife.xy-asia.com`.
- Большинство «недостающих» у консультантов = поддомены qq.com/gtimg.com (уже покрыты корнями) — не дублировал. `smtcdns.*`/`myqcloud.com` оставлены (Grok советовал убрать `smtcdns` — отклонено).

**RAM Happ.**
- Рост 14-16→29МБ — внешний (обновился Happ → распухли встроенные geoip.dat/geosite.dat), конфиг не менялся.
...
## 22. Изменения 2026-09-07 (клиентский DNS: DoH по IP + fail-open резерв)

**Тестов: 194/194.** Правка только в `build_client_xray_config` (`cascade/xray_config.py`), секция `dns.servers`. Сервер выхода, мост, роутинг, ключи и порты не тронуты — клиентские ссылки и профили не ломаются.

**Дефект 1 — bootstrap DoH.** DoH был задан доменом (`https://dns.google/dns-query`). Домен не подпадал ни под одно правило `dns.servers` (не `geosite:category-ru`, не `.ru/.su/.рф`, не WeChat), секции `hosts` в профиле нет — значит Xray резолвил его **системным DNS устройства**, то есть резолвером мобильного оператора, который ТСПУ подменяет. Не поднялся резолв одного домена — не поднялся весь DoH. Риск был записан ещё в §9 и не проверялся.
**Фикс:** DoH задан IP — `https://8.8.8.8/dns-query`. Резолвить нечего, шаг исчезает. Документация Xray прямо разрешает DoH по IP. Вариант с `hosts` отклонён: по докам для DoH, заданного доменом, адрес в local-режиме резолвится системой мимо `hosts` — то есть подпорка ненадёжна, а замена на IP закрывает вопрос целиком.

**Дефект 2 — РФ-трафик умирал вместе с мостом.** DoH ходит ВНУТРЬ туннеля. При `domainStrategy: IPIfNonMatch` домен без совпадения по имени сперва резолвится, и только потом проверяется `geoip:ru`. Значит любой РФ-сервис, которого нет ни в `.ru/.su/.рф`, ни в урезанном `geosite.dat` внутри Happ (РФ-сервисы на `.com`, РФ-CDN), требовал DNS-запроса через мост. Мост лёг — запрос висит — сайт не открывается, хотя после резолва пошёл бы `direct` мимо моста. Владелец наблюдал этот симптом вживую.
**Фикс (решение владельца — fail-open):** в конец `servers` добавлен безфильтровый `77.88.8.8`. По документации Xray безфильтровые серверы опрашиваются последовательно в порядке конфига, следующий берётся при провале предыдущего — значит при живом туннеле резерв не задействуется вообще, а включается только после провала DoH. Яндекс в `geoip:ru` → запрос уходит `direct`, мимо моста.

**Принятые риски (осознанно, verifier-adversarial):**
- При мёртвом DoH зарубежный домен резолвит Яндекс. Если тот вернёт РФ-адрес (РКН-заглушка или РФ-узел CDN), правило `direct` по `geoip:ru` уведёт сайт мимо каскада — пользователь увидит заглушку вместо сайта. Актуально только когда туннель уже не работает; при живом туннеле поведение не меняется. Симптом для мониторинга: жалобы «зарубежные сайты открываются неправильно».
- При мёртвом туннеле первый резолв ждёт таймаута DoH (~5-10 с), потом уходит к Яндексу. Раньше на этом месте не открывалось ничего.

**Как доезжает до клиентов.** Профиль генерируется на каждый запрос `/sub/<token>`, интервал автообновления 3 ч — клиенты на подписке подтянут сами, переимпорт не нужен. Кто сидит на вручную импортированном JSON — останется на старом профиле со старым bootstrap-риском, ему нужен переимпорт. Голых `vless://`-ссылок правка не касается (в них DNS не передаётся).

**Обе части проверены верификаторами:** verifier-spec — PASS по всем пунктам; verifier-adversarial — деплой одобрен, риски выше.

**Открытый хвост — симптом «при мёртвом мосте умирает и РФ-трафик» не объяснён.** Владелец наблюдал: когда мост недоступен, не грузятся и российские сайты, включая зону `.ru`, а выключение Happ чинит всё мгновенно. По конфигу этого быть не должно: `.ru` совпадает с правилом `domain:ru` по имени, резолвится Яндексом напрямую, туннель в цепочке не участвует. Значит ломается ВЫШЕ нашей DNS-логики. Причина: `direct` не означает «мимо Happ» — при включённом клиенте iOS заворачивает весь трафик в его tun, и российский пакет всё равно проходит через ядро Xray внутри Happ, просто уходит в outbound `freedom`. **Happ — единая точка отказа для всего трафика, включая РФ.** Если ядро встало или клиент пересоздаёт туннель, потеряв основной outbound, не проходит ничего, и до правил роутинга дело не доходит. Проверить по исходникам нельзя — Happ закрытый (§20).
Из этого следует: **правка §22 этот симптом может не лечить**, она закрывает другую (доказанную) дыру — bootstrap-резолв. Гипотеза «DNS-модуль захлёбывался на висящих DoH-запросах» ослаблена: владелец пользуется телефоном легко (Telegram + РФ-приложения), десятков параллельных резолвов нет.
**Решающий тест (не проведён):** собрать профиль с заведомо мёртвым адресом моста и открыть с ним российский сайт.
```
cd /opt/cascade && python3 -c 'from cascade.config import load_config,CONFIG_PATH,primary_exit;from cascade.xray_config import client_profile_json;c=load_config(CONFIG_PATH);ex=primary_exit(c);cl=c.clients[0];open("/root/test-dead-bridge.json","w").write(client_profile_json(cl.uuid,ex,"192.0.2.1",remarks="TEST-DEAD-BRIDGE",fingerprint=c.fingerprint));print(cl.name)'
```
Грузится → горлышком был DNS, §22 попала в цель. Не грузится → клиент кладёт всё целиком, копать надо в Happ, а не в конфиге. (Похожие профили владелец готовил 2026-08-22, лежат в `.superpowers/cascade-failopen-test/`, результат нигде не записан.)

**Доставка правок клиентам — не по интервалу.** Панель отдаёт `profile-update-interval: 3`, но Happ его не соблюдает: замер по `sub_update_count` за 67 дней дал максимум 2,6 обновления в сутки у самого активного клиента при ожидаемых 8, а часть клиентов не запрашивала подписку ни разу. Обновляется, судя по всему, по событию (открытие приложения), а не по таймеру. Планировать раскатку «за 3 часа» нельзя — проверять по счётчику, кто реально забрал.

---

## 23. Изменения 2026-09-11 (панель и подписка на TLS 1.2)

**Правка только в `caddyfile()` (`cascade/panel/deploy.py`)** — генератор конфига Caddy для панели. Серверов выхода, моста, роутинга, ключей и портов не касается.

**Симптом.** Техдомен панели периодически недоступен: без VPN не открывается, через VPN открывается; с компьютера работает, с телефона нет. Наблюдалось владельцем и было зафиксировано ещё 2026-07-01 (`docs/research/2026-07-01-tls-blocked-from-mobile.md`), но тогда причину определили неверно.

**Прежняя (ошибочная) версия.** Считали, что мобильные операторы блокируют CRL/OCSP Let's Encrypt, и пошли менять удостоверяющий центр на ZeroSSL. Путь умер: ZeroSSL не выдаёт сертификаты на `.ru`. Код `setup_acme_cert`/`setup_auto_renewal` из того захода в репозиторий так и не попал — в `deploy.py` его нет, хотя research-док 2026-07-01 называет его готовым.

**Текущая версия.** Промежуточные сетевые устройства в РФ не доводят до сервера отдельные варианты TLS ClientHello, в первую очередь от TLS 1.3. Совпадает и с нашим июльским наблюдением (порт 443 доступен, хендшейк начинается и не завершается), и с базой знаний хостера AdminVPS (статья 559). Один источник — не доказательство, но лечится дёшево и проверяемо.

**Что сделано.** В блок сайта добавлено:
```
tls {
    protocols tls1.2 tls1.2
}
```
Автоматический ACME не отключается — блок `tls` задаёт только версии протокола.

**Почему это безопасно для VPN.** Клиент подключается к мосту **по IP на высокий relay-порт** (`client_profile_json`: `host = relay_ip`, `port = relay_port`), попадает сразу в DNAT и уходит на выход. Домен, порт 443, nginx и Caddy на пути VPN не участвуют. Reality построен на TLS 1.3 + X25519 (мы и SNI-кандидатов отбираем по TLS1.3+X25519+h2) — там 1.3 обязан остаться.

**Побочная гипотеза, которую эта правка проверяет.** Если у клиента оператор рвёт TLS 1.3 по 443, то VPN у него работает, а подписка по домену не скачивается — молча. Это может объяснять низкие `sub_update_count` лучше, чем «Happ не соблюдает интервал» (§22). База цифр на 07.09.2026 записана в `docs/research/2026-09-07-nlm-september-dpi-changes.md` — сравнить после деплоя.

**HTTP/3.** Caddy v2 включает его по умолчанию, но на :443 у нас nginx stream (TCP), UDP наружу не открыт — QUIC до Caddy не доходит. Отключать пришлось бы глобальным блоком в корневом Caddyfile, который мы не переписываем принципиально. Оставлено как есть.

**Ловушка деплоя (исправлена в dev-заметках).** Записанная ранее команда регенерации Caddyfile не передавала `caddy_bind_port`, а на мосту Caddy сидит на :8443 за nginx (§16). Выполнение её как было записано заставило бы Caddy занимать :443, который держит nginx, и уронило бы панель. Теперь порт определяется из текущего файла.

**Результат деплоя 2026-09-11 — предварительный.** Конфиг раскатан (`bind port: 8443`, `Valid configuration`, reload без разрыва). Владелец проверил с телефона без VPN: домен «вроде открывается». **Подтверждением это не считать** — симптом перемежающийся и зависит от маршрута, единичное удачное открытие может быть совпадением. Твёрдую проверку даст `sub_update_count`: база на 07.09.2026 записана в `docs/research/2026-09-07-nlm-september-dpi-changes.md`; если счётчик у клиента, не обновлявшегося с июля, сдвинется сам собой — причина была в TLS 1.3, и заодно подтвердится, что часть клиентов физически не могла скачать подписку.
